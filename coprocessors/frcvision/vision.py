      ...
      try:
         ...
         #   lastLedTime = currentLoopTime
          ...
          if idx > 9:
            ...
            averageGrabTime = sum( grabTimes ) * 1000 / len( grabTimes )
            averageProcTime = sum( processTimes ) * 1000 / len( processTimes )
            timesTable.putNumber( 'val', averageTime )
            print( 'averageTime: {:>5.1f}  grab: {:>5.1f}  proc: {:>5.1f}'.format( averageTime, averageGrabTime, averageProcTime ) )


          if False:
             frame_HSV = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
             frame_threshold = cv2.inRange(frame_HSV, (50, 0, 220), (200, 50, 255))
             ...
             cv2.circle(
                 img,
                 (int(width / 2 - 14), int(height / 2 - 14)),
                 int(3),
                 (0, 0, 255),
                 int(3),
                 int(8)
             ...
             cv2.line(
                 img,
                 ( int(width / 2 + 14), int(height / 2 + 3) ),
                 ( int(width / 2 + 13), int(height / 2 + 7) ),
                 (0, 0, 255),
                 int(3),
                 int(8),
                 int(0)
             ...
             cv2.line(
                 img,
                 ( int(width / 2 + 13), int(height / 2 + 7) ),
                 ( int(width / 2 + 11), int(height / 2 + 10) ),
                 (0, 0, 255),
                 int(3),
                 int(8),
                 int(0)
          ...
          if len( pipeline.filter_contours_output ) > 0:
              ...
              #print( pipeline.find_contours_output[0] )
              #center = ( int(width / 2), int(height / 2) )
              #cv2.circle(
              #    img,
              #    center,
